...
const withLegacyPath = <T extends { source: string }>(asset: T): T & { path: string } => ({
  ...asset,
  path: asset.source
});
...
export const ASSETS = {
  spritesheets: {
    player: withLegacyPath({
      key: 'player-sheet',
      source: svgDataUri(playerSheetSvg),
      frameConfig: { frameWidth: 32, frameHeight: 32 }
    }),
    enemyCart: withLegacyPath({
  ...
  images: {
    ...
    arenaBackground: withLegacyPath({
      key: 'arena-bg',
...
export const getAssetSource = (asset: { source: string; path?: string }): string => asset.source || asset.path || '';
